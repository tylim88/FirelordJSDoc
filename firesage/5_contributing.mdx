...
6. Query Constraint type affect Read type. For example if we have `orderByChild('a'), equalTo(true)`, then we can further narrow down the type of child node `a` to only `true`.
