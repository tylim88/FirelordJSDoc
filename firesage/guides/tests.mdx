...
We don't recommend you run the test with only emulator, becaus emulator in local but not in CI (tested in Github Action). Aparrently Firebase unable to pick up the database rule defined in firebase.json. `@firebase/rules-unit-testing` can pick up database rule because the it can pick up database rules using `fs`, see example below for details.
...
This is different from Firestore-emulator interaction.
::
