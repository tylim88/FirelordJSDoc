...
Now, we lost some data because we discarded `undefined`, no, we don't. This is because `undefined` is not a valid RTDB data, we can read something as `undefined`, but we never able to write something as `undefined`.
