...
2. If it is an array, convert it to object, else do nothing. <-- FireSageJS implements this
...
Now we know that `undefined` is safe to discard, what about lenght? Well, `Object.keys({ 0: 'a', 2: 'c', 4: 'e' })`, we get `3` and we do have `3` meaningful data points.

FireSageJS never read data as array.
