...
The library size on disk is 300KB, most of the code are type definitions and comments, the runtime code is at mere 5KB after minified and gzipped, [see how we got this number](/minified_size).
...
;async () => {
	...
	const snapshot3 = await get(
		query(
			...
			// descendants node are 'h', 'j' and 'j/k'
			// however 'j' is not a valid type because the type of node 'j' is non-primitive
