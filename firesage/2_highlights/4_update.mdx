...
:::note
The implementation of `update` is largely different than the official SDK's update due to [deficiency](https://stackoverflow.com/questions/73263183/typescript-type-of-key-is-string-if-key-is-template-literal-a-string) in Typescript.

Luckily arrays as parameters are not bad at all, personally I find this is more enjoyable.
:::

:::info
`onDiconnect`'s `update` works like normal `update`.
:::
