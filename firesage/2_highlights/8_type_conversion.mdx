...
| `NumericKeyRecord<T>` | Record\<`${number}`, converted\<T\>\> | Record\<`${number}`, converted\<T\>\> \| converted\<T\>[] | invalid           |
| `PushAbleOnly<T>`     | `Record<string, converted<T>>`        | `Record<string, converted<T>>`                            | invalid           |
| `PushAble<T>`         | `Record<string, converted<T>>`        | `Record<string, converted<T>>`                            | invalid           |
