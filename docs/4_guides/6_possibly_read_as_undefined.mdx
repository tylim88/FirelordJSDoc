...
However set merge(setDoc with merge) update the doc if the doc exists and create it if it does not exist, it is similar to `upsert` in SQL land.
...
This is where `PossiblyReadAsUndefined` is useful, however it is not a good solution, a good solution is to forbid set merge and create a field type that allow use to use set merge on certain field, kind of like how delete field works.
