...
// the collection path name is full collection path name(basically full document path minus the last document ID)
// for example:
// if your full document path is "User/UserId", the collection path name is "User"
// if your full document path is "User/UserId/Item/ItemId", the collection path name is "User/UserId/Item"
// this value is typed, FirelordJS also check the number of forward slash
```
