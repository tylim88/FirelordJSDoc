...
// Recommendation: Export this if the collection is sub-collection and then fill in collection path later, because sub collection most likely have multiple dynamic document ID.
export const firelordExample = getFirelord<Example>(db)
// OR
export const firelordExample_Alt = getFirelord<Example>()

// Recommendation: Export this if the collection is root collection
export const example = firelordExample('SomeCollectionPath')
// argument is collection path (basically document path minus the last document ID segment)
// for example:
// if your document path is "User/UserId", the collection path name is "User"
// if your document path is "User/UserId/Item/ItemId", the collection path name is "User/UserId/Item"
// this paramater is typed, FirelordJS also check the number of forward slash so you wont mess up the number of segments.
```
