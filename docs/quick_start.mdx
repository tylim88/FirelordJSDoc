...
// Recommendation: Export this if the collection is sub-collection and then fill in the value later, because sub collection most likely have multiple dynamic document ID.
...
// the collection path name document path minus the last document ID segment
// for example:
...
// this paramater is typed, FirelordJS also check the number of forward slash so you wont messed up the number of segments.
```
