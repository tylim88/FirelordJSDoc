...
// Recommendation: Export this if the collection is sub-collection and then fill in the value later, because sub collection may have multiple dynamic document ID.
...
export const example = firelordExample('SomeCollectionName') // the value is guided by Typescript
```
