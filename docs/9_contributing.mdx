...
7. Query Constraint type affect Read type. For example if we have `where('a', '==', true)`, then we can further narrow down the type of field `a` to only `true`.
...
PROJECT_ID="xxx"
PRIVATE_KEY_ID="xxx"
PRIVATE_KEY='"-----BEGIN PRIVATE KEY-----\nXXX\n-----END PRIVATE KEY-----\n"' # yup, must enclosed inside single quotes
CLIENT_EMAIL="xxx"
CLIENT_ID="xxx"
AUTH_URI="xxx"
TOKEN_URI="xxx"
