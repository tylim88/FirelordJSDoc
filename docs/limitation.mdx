...
1 and 2 may be supported in the future, 3 doesn't make sense to support it
...
2. cursor rest argument must need tuple type to work, it will rejects all array type, resulting in type error.
   - What this mean is, for some reason you want to have computed array as cursor rest argument(maybe you are doing some high level abstraction), it will never pass the type check.
   ...
   - It is still safe to run.

1 may has remedy in future, 2 see no possibility to improve at current stage.
