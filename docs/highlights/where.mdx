...
FirelordJS will still make sure the value type is correct, just no autocomplete.

You can access the `MetaType` using **[index accessor](./metatype#index-accessor-%EF%B8%8F)** to check what type you need.
...
<Image
	...
	text={`FirelordJS displays error mesagae in hint if it detects any invalid character.`}
/>
...
<Image
	...
	text={`FirelordJS displays error mesagae in hint if it detects path is wrong.`}
/>

<Image
	src="https://github.com/tylim88/FirelordJSDoc/blob/main/static/img/where5.png?raw=true"
	text={`FirelordJS counts and compares the number of slash and displays it in hint`}
/>
