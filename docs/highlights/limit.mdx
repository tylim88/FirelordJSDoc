...
`limitToLast` type works like `limit` type, but with one extra catch: at least one `orderBy` clause is needed.

<Image
	src="https://github.com/tylim88/FirelordJSDoc/blob/main/static/img/limit2.png?raw=true"
	text="limitToLast"
/>

### Multiple Clause

Only the last one is effective, this is also how the original sdk work.
