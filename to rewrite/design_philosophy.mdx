...
Firestore SDK also comes with quirky runtime behaviors that ready to kick your ass and ruin your wet dream:

1. Limit clause throw on negative and 0 value. Throw on negative is good but throw on 0 value is arguable because 0 can be resulted from correct computation.
2. In some case, update behave partially like set, resulting in field deletion, danger!
3. Let not start with how [naughty](https://stackoverflow.com/questions/71171678/firestore-should-we-avoid-using-value-type-null) `null` is in query....
4. All kinds of unguarded limitations(known but does not bother to do anything about it) for us to explore, adventures, yeah!

and more...

An unsafe code is a huge technical debt, it creates problem in a dark corner, you need to revisit it constantly and put more works to make it safer.
...
1. Familiarity (a.k.a knowledge reuse):
   ...
   - The wrapper API must be at least identical to the official Firestore SDK and can only be simpler onward.
   - The wrapper runtime behavior must be at least identical to the official Firestore SDK and can only be more intuitive onward.
   - Benefits:
     - Greatly reduce the learning cost, developers can simply reuse the knowledge they have learned in the official API.
     - No surprise behavior in runtime.
     - If developers decided not to use the wrapper anymore, they can easily fallback to the official API.
...
3. Always choose non invasive solution if possible.
   ...
   - Only alter the runtime behavior to be more intuitive if the official API runtime behavior is quirky(high cognitive cost).
...
4. Prevent the preventable.
   ...
   - There is not much thing we can do to system and environment exceptions but we can do something to inputs exceptions.
