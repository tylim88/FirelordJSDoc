...
| Base                                | Read                          | Write                  | Compare                |
| ----------------------------------- | ----------------------------- | ---------------------- | ---------------------- |
| `TimestampPersist` \| `DatePersist` | `TimestampPersist`            | `Timestamp` \| `Date`  | `Timestamp` \| `Date`  |
| `ServerTimestampPersist`            | `TimestampPersist`            | `ServerTimestamp`      | `Timestamp` \| `Date`  |
| `GeoPointPersist`                   | `GeoPointPersist`             | `GeoPoint`             | `GeoPoint`             |
| `DocumentReferencePersist<M>`       | `DocumentReferencePersist<M>` | `DocumentReference<M>` | `DocumentReference<M>` |

Serializing object to JSON will strip all the methods and functions, the persistent types keep only valid JSON type.

Use persistent types if you plan to serialize object to JSON.

Use case example: Persist `Timestamp` in browser local storage, the read type is `{seconds:number, nanoseconds:number}`, no methods.
