...
Use case example: Persist `Timestamp` in browser local storage using tools like `redux-persist` or `zustand-persist` or just `JSON.stringify()`, the read type is `{ seconds:number, nanoseconds:number }`, no methods like `toDate` or `to Milis`.
