...
| `JSONDocumentReference<M>`    | `JSONDocumentReference<M>` | `DocumentReference<M>` | `DocumentReference<M>` |

Serializing object to JSON will strip all the methods and functions, the JSON types keep only proprty with valid JSON type.
...
May provide custom solution in the future
:::
