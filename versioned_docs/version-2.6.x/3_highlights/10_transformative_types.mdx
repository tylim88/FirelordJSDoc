...
Use case example: Persist `Timestamp` in browser local storage using tools like `redux-persist` or `zustand-persist`, the read type is `{seconds:number, nanoseconds:number}`, no methods.
