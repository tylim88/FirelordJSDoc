...
When using `where` with `documentId`, there would be no auto complete for the value argument(`where` clause third argument) because we need to make way for more type checks.
