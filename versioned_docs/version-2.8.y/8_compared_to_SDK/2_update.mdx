...
<div align="center">
	<img src="/firelordv2.6/update1.png" />
</div>
<div align="center">
...
2. Accept unknown(excess) member from stale value,
   resulting in storing unnecessary information in Firestore.
...
3. Accept `undefined` but `undefined` is not a valid Firestore value.
...
But apparently it has [use cases](https://github.com/tylim88/Firelord/issues/20#issuecomment-1545975918), so we added `transaction.updateNoFlatten`, `batch.updateNoFlatten`, and `updateDocNoFlatten` for those who needs the original behavior.
