...
9. Support compile to ESM.

10. New Logo

11. add tests for DocumentReference, Bytes and GeoPoint data type
...
Despite PROJECT_ID is not a secret, we should hide it(don't upload to Github) because we want to allow restriction free read and write for smooth testing.
...
3. `npm run d-p-test` to test published code(You can specify which version to test by changing `version` in `package.json`).
