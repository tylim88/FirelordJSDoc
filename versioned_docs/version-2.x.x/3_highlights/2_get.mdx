...
Firestore does not have distinct type for server timestamp, all field values in Firestore have the same type, that is `FieldValue`, read **[Inhomogeneous Field Values](/guides/inhomogeneous_field_values)** for more info.
