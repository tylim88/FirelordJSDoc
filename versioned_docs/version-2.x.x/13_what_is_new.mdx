...
Finally, we need to call `getFirelord` twice to create `FirelordRef` because `getFirelord` is a curried function.

Curried function is useful if we have more than one Firestore instance, but most applications need only one instance, so curry provides no real value other than increasing cognitive cost.
