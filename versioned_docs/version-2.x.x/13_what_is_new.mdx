...
In order to reuse child collections' FirelordRef, we need to wrap it in another function. This is due to the dynamic nature of document IDs.

This results in how we export top-level ref being different than how we export child ref. This feels awkward when we try to structure our code.

Also, neither `docRef`,`colRef` nor `colGroupRef` of child collections are intuitive to create, especially `colGroupRef`.

Despite `colGroupRef` not needing any document IDs, we still need to supply two documentIDs to it.

Finally, we need to call `getFirelord` twice to create a `FirelordRef` because `getFirelord`` is a curry function.
