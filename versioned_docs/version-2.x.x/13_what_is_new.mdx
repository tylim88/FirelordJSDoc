...
In V1, working with top collections is a pleasant but not so much with child collections.

In order to resue child colloection `FirelordRef`, we need to wrap it in another function because in most cases document IDs are dynamic.
...
How we export top level ref is different than how we export child ref and this is harmful to code structure.
