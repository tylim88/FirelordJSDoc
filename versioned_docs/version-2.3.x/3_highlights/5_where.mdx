...
// case where even const assertion does not work!
// v3.0.0 will address these issues
query(ColRef, where('b', '==', ['a'] as const))
query(ColRef, where('b', 'in', [['a'] as const]))
query(ColRef, where('b', 'in', [['a']] as const))
```

<Image src="/firelordv2/where4.png" text={`const assertion`} />

As you can see, depend on how you do const assertion, there are still some cases that cannot be handled.

V3 will solve issue like this, so please be patient.
