...
| Field Value | Interface                                                                 | Signature                                  | Note                                                | Assignable |
| ----------- | ------------------------------------------------------------------------- | ------------------------------------------ | --------------------------------------------------- | ---------- |
| `delete`    | [`DeleteField`](../4_guides/3_delete_field.mdx)                           | `{ 'Firelord.FieldValue': unique symbol }` | allow a field to be read as undefiend and deletable | Yes        |
| none        | [`PossiblyReadAsUndefined`](../4_guides/5_possibly_read_as_undefined.mdx) | `{ 'Firelord.FieldValue': unique symbol }` | allow a field to be read as undefiend               | Yes        |
